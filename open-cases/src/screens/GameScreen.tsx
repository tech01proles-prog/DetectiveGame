...
function CaseTab({loc,state,action,setFinalOpen,canFinal,scenario}:{loc:any;state:GameState;action:(id:string)=>void;setFinalOpen:(v:boolean)=>void;canFinal:boolean;scenario:ScenarioData}){const open=state.discoveredLocationIds.includes(loc.id);const getCharacter = (id: string) => scenario.characters.find(c => c.id === id);const scenarioId = scenario.scenario.id;return <><div className="panel-title selected-location-heading"><span className="eyebrow"><MapPin size={12}/> ВЫБРАННАЯ ТОЧКА</span><h2>{loc.title}</h2><p>{loc.address}</p></div><div className="location-hero"><img src={resolveImageUrl(scenarioId, loc.image)}/><span>{loc.category}</span><div className="hero-badge">{open?'ДОСТУПНО':'НЕ ИССЛЕДОВАНО'}</div></div><div className="panel-copy"><p>{loc.description}</p></div>{!open?<div className="locked-card"><Lock size={18}/><b>Локация пока закрыта</b><span>{loc.lockedReason}</span></div>:<div className="actions"><span className="section-label">Доступные действия</span>{loc.actions.map((a:any)=>{const done=state.completedActionIds.includes(a.id),blocked=a.requiresClueIds?.some((id:string)=>!state.foundClueIds.includes(id));const Icon=getActionIcon(a.id,a);return <button key={a.id} disabled={done||blocked} onClick={()=>action(a.id)} className="action-card"><div className="action-icon">{done?<Check size={17}/>:<Icon size={17}/>}</div><div><b>{a.label}</b><span>{done?'Завершено':blocked?'Сначала найдите нужную зацепку.':a.description}</span></div><ChevronRight size={17}/></button>})}</div>}<div className="location-people">{loc.characterIds.length>0&&<><span className="section-label">Персонажи в точке</span>{loc.characterIds.map((id:string)=>{const c=getCharacter(id)!;return <div className="mini-person" key={id}><img src={resolveImageUrl(scenarioId, c.portrait)}/><div><b>{c.name}</b><span>{c.relation}</span></div></div>})}</>}</div><button className={`final-btn ${canFinal?'ready':''}`} disabled={!canFinal} onClick={()=>setFinalOpen(true)}><FileText size={18}/>{canFinal?'Сформировать версию расследования':'Соберите минимум 10 улик'}</button></>}
function PeopleTab({state,select,scenario}:{state:GameState;select:(id:string)=>void;scenario:ScenarioData}){const characters = scenario.characters;const scenarioId = scenario.scenario.id; return <><div className="panel-title"><span className="eyebrow">ДОСЬЕ</span><h2>Люди</h2><p>{characters.length} фигур в деле</p></div><div className="people-list">{characters.map((c:any)=><button key={c.id} className="person-card" onClick={()=>c.locationId&&select(c.locationId)}><img src={resolveImageUrl(scenarioId, c.portrait)}/><div><b>{c.name}</b><span>{c.age} · {c.role}</span><small>{state.questionedCharacterIds.includes(c.id)?'Опрошен':'Не опрошен'}</small></div><ChevronRight size={16}/></button>)}</div></>}
...
function DialogueModal({node,scenario,onClose,onChoose}:{node:DialogueNode;scenario:ScenarioData;onClose:()=>void;onChoose:(c:any)=>void}){const character = scenario.characters.find(c => c.id === node.characterId)!;const scenarioId = scenario.scenario.id;return <div className="modal-backdrop dialogue-backdrop"><div className="modal dialogue-modal" onClick={e=>e.stopPropagation()}><div className="dialogue-top"><div><span className="eyebrow">{node.eyebrow}</span><h2>{node.title}</h2></div><button onClick={onClose}>×</button></div><div className="speaker"><img src={resolveImageUrl(scenarioId, character.portrait)}/><div><b>{character.name}</b><span>{character.role}</span></div></div><div className="dialogue-lines">{node.lines.map((line,i)=><p key={i}>{line}</p>)}</div><div className="choice-list"><span className="section-label">Что спросить дальше</span>{node.choices.map(c=><button className="dialogue-choice" key={c.id} onClick={()=>onChoose(c)}><div><b>{c.label}</b><span>{c.text||'Продолжить разговор и проверить показания.'}</span></div><ChevronRight size={17}/></button>)}</div><div className="dialogue-foot">{node.intro}</div></div></div>}
